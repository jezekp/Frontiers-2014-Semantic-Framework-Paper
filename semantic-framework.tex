...
Our research group specializes in research of brain electrical activity and operates a laboratory in which the techniques and methods of electroencephalography (EEG) and event related potentials (ERP)are widely used. In addition to the experimental work, mostly focused on recording and analysis of cognitive event related potentials, we are working on the development of the software and hardware infrastructure for research in electrophysiology~\cite{10.3389/fninf.2014.00020}. Since experimental work is very time consuming and vast amounts of experimental data are produced at various stages of processing from primary data recording to final interpretation of data, means supporting the semantic description of these data and software systems improving the storage, management and sharing of data (at least within the research group) would significantly increase the research efficiency in the field. This need is more crucial when researchers want to share their data, processing steps or even interpretation results within wider community.

The semantics of (not only) structured electrophysiological data means that these data are accompanied by metadata that specify their meaning. Then the most pressing difficulties are not only an absence of standardized and generally used data formats for raw data, but also well-defined metadata descriptions (as described e.g. in~\cite{data-sharing-for-computational-neuroscience}). Metadata can be usually seen as the links to a~specific dictionary, in which the data are explained and defined by using e.g. the plain textual description, sets of values or even formal logic. Then the richness and accuracy of semantic content of these dictionaries determines the level of semantic description of data. Currently, a variety of experimental approaches, used techniques, targeted subjects, usable hardware and software infrastructures etc. lead to the accumulation of heterogeneous data that are stored in various, often proprietary, data formats, and annotated differently in both scope and quality sense. Moreover, these data are stored in various repositories including files and databases of different types and annotated respecting limitations of used technologies. Then, if we take into account these conceptual and technological heterogeneities and not give up efforts to increase research efficiency in the field by sharing computational resources like raw data, metadata, processing methods and workflows among laboratories, the long term storage, management and sharing of experimental data and related metadata is the first important step to achieve this goal. Improvement of data sharing requires providing data in standardized (or at least widely used) data formats and also providing related metadata in standardized semantic structures. The abstract level of description of such data formats and metadata structures is currently widely discussed in the community (e.g. within INCF data sharing activities). A well-specified level of these data formats and metadata structures could bring real sharing opportunities in the domain and at the same time it should not prevent anyone from the community to provide data and metadata in these formats and structures. Although this decision is very important for the future of data sharing in the domain, it is out of scope of this article.
