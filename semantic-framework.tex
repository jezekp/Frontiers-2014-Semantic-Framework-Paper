...
\section{}
  The goal of this article is to contribute to the efficiency of research in an electrophysiological domain by improving opportunities for describing semantics of structured electrophysiological data. The research efficiency can be, among other measures, expressed as an ability of data owners and data users to understand experimental data in a longer term. The appropriate semantic description of these data is then closely related to the successful long-term management and sharing of data. Improvement of data management and sharing requires compliance with the minimum standards for data formats and metadata structures. The abstract level of semantics used for describing such data formats and metadata structures, which is widely discussed in the community, has to be accompanied by selecting suitable semantic expressive means.
...
\caption{Prot\'{e}ge Visualization. The left column shows ontology classes. All classes are subclasses of a superclass \emph{Thing}. The right window shows the class \emph{Person}. The class annotations (e.g. \emph{Comment} or \emph{EquivalentClass}) and properties annotations (e.g. \emph{researchGroup} - each person is a member of at least one research group) and class instances are transformed from the enriched JavaBean (see Listing \ref{Annotated_Java_Bean_Example}).}
