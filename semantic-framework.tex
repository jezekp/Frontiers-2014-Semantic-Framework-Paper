...
\caption{Prot\'{e}ge Visualization. The left column shows ontology classes. All classes are subclasses of a~superclass \emph{Thing}. The right window shows the class \emph{Person}. The class descriptions (\emph{EquivalentClass}) and properties  (e.g. \emph{researchGroups} - each person is a~member of at least one research group) are transformed from the enriched JavaBean (see Listing \ref{Annotated_Java_Bean_Example}). The class instances (e.g. Person_0) are originally obtained from the relational database.}
