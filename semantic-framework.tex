...
\title[Mapping Object-Oriented Structures to Semantic Web Technologies]{Semantic Framework}
...
Despite the fact that the World Wide Web (WWW) is the largest knowledge database available for human readers, containing a large collection of unclassified information, it is extremely difficult to handle this enormous amount of information.
...
The time complexity of the algorithm is crucial when the Semantic Framework is deployed in applications. When we analyze time complexity, we consider the \emph{transformation} operations and ignore the \emph{preprocessing} operations (e.g obtaining JavaBeans from the database). Algorithm \ref{alg:Translation JavaBeans to Ontology} describing translation of one instance of extended JavaBean structure. It consists of three cycles:

\begin{itemize}
\item Extracting classes
\item Extracting properties of partial classes
\item Extracting annotations of classes and their properties
\end{itemize}
...
\tiny
 ...
 \caption{Translation JavaBeans to Ontology}
 \label{alg:Translation JavaBeans to Ontology}
 \begin{algorithmic}[1]
...
$JB=(C{}_{\text{JB}}, P{}_{\text{JB}}, A{}_{\text{JB}})$ \textbf{where}

\hspace{10mm}$C{}_{\text{JB}}$: classes,

\hspace{10mm}$P{}_{\text{JB}}$: properties,

\hspace{10mm}$A{}_{\text{JB}}$: annotations of given JavaBean
}
...
$O=(C{}_{\text{O}}, O{}_{\text{O}}, D{}_{\text{O}}, R{}_{\text{O}}, S{}_{\text{O}})$ \textbf{where}

\hspace{10mm}$C{}_{\text{O}}$: classes,

\hspace{10mm}$O{}_{\text{O}}$: object properties,

\hspace{10mm}$D{}_{\text{O}}$: datatype properties,

\hspace{10mm}$R{}_{\text{O}}$: restrictions,

\hspace{10mm}$S{}_{\text{O}}$: subclasses of given ontology
}
 ...
 {
 \ForAll{classes C $\in$ $C{}_{\text{JB}}$} {
       \State create a class $\phi(C) \in$ O

    \ForAll{properties P $\in$$P{}_{\text{JB}}$} {
       \If{P $\in$ T where T $\in \left\{{real, integer, boolean, enum, set, string}\right\}$ $\to$ primitive type}
           \State create a DataType property $\phi(D) \in$ O
         \Else
           \State create an Object property $\phi(O) \in$ O
       \EndIf

    \ForAll{annotations A $\in$$A{}_{\text{JB}}$}{
       \If{A $\in$ N where N $\in \left\{{@differenFrom, @equvivalentProperty}\right\}$}
         \State create a subproperty $\phi(P) \in \left\{{  C{}_{\text{O}}, P{}_{\text{O}} }\right\} \in$ O
       \Else {
           \State create a subclass $\phi(S) \in \left\{{  C{}_{\text{O}}, P{}_{\text{O}} }\right\} \in$ O
           \State create a restriction $\phi(R) \in \left\{{  C{}_{\text{O}}, P{}_{\text{O}} }\right\} \in$ O
           }
       \EndIf
   } \EndFor
...
The extraction of classes (lines 1 to 3)  takes  $N{}_{\text{C}}$ operations. The extraction of properties takes (lines 4 to 8) $N{}_{\text{P}}$ operations. The extraction of class or properties annotations (lines 9 to 16) takes in the worst case N+M (when the \emph{else} branch is executed) operations. We can simplify it and substitute $2N{}_{\text{A}}$ for it.

Summing up the operations above, the amount of work done by the algorithm is:

\begin{equation}
F(N)_{inst}=N{}_{\text{C}}*N{}_{\text{P}}*2N{}_{\text{A}}\approx 2N^3
\label{eq:algorithm complexity}
\end{equation}

From Equation \ref{eq:algorithm complexity} it can be inferred that the worst-case complexity of the translation algorithm is O($N^3$) when an one instance of a JavaBean is transformed. When we transform more instances (denote it \emph{n}) and amount of work to transform an one instance remains constant (denote it $C$) then the amount of work done is such as expressed in Equation \ref{eq:algorithm complexity_simply}. Then, the complexity of the transformation algorithm is O(N).

\begin{equation}
F(N)=n*C\approx N
\label{eq:algorithm complexity_simply}
\end{equation}

\subsection{Experimental Verification}
\label{Experimental_Verification}

We verified the complexity of the algorithm described in Equation \ref{eq:algorithm complexity_simply} in a practical scenario. Firstly, we prepared a set of \emph{Experiment} instances. Then, we assigned instances of \emph{Person}, \emph{Scenario}, \emph{Hardware} and \emph{Data} to each experiment. The class \emph{Person} was extended by the set of supported annotations from Table \ref{tab:Java annotations to OWL mapping}. The tests were run ten times and the result was created as an average of partial results.
